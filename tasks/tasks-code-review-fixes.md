...
- [ ] 4.0 Move client-side mutations to authenticated Server Actions (Issues #10, #11)
  ...
  - [ ] 4.4 **Update AddRoomForm.tsx:** Refactor `src/app/(admin)/admin/rooms/components/AddRoomForm.tsx` to call the new `addRoom` Server Action instead of directly inserting via the client-side Supabase SDK (`supabase.from("rooms").insert(...)`). Remove the `import { supabase } from "@/lib/supabase/client"` line.
