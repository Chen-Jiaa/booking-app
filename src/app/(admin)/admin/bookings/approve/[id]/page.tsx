import { approveBooking } from "@/app/(admin)/actions/approve-reject-booking";
import { formatBookingDate, formatBookingTime } from "@/lib/date-utils";
import { getPurposeLabel } from "@/lib/getPurposeLabel";
import Link from "next/link";

export default async function ApproveBookingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!result.success) {
    return <p>Failed to approve booking: {result.error}</p>;
  }
  ...
  return (
    <div className="flex justify-center">
      <div className="mx-auto max-w-md rounded-lg border p-6 text-center">
        <h1 className="mb-2 text-xl font-semibold">Booking Approved ✅</h1>
