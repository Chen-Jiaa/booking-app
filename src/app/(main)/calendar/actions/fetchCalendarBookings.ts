...
export async function fetchCalendarBookings(
  ...
  const [user, { data: standardData }] = await Promise.all([
    ...
    supabase
      .from("bookings")
      .select(
        "id, room_id, room_name, start_time, end_time, status, name, phone, purpose, event_name, client_name, expected_attendance",
      )
  ...
  const { data: multiDayData } = await supabase
    .from("bookings")
    .select(
      "id, room_id, room_name, status, name, phone, purpose, event_name, client_name, expected_attendance, start_time, end_time",
    )
    .in("status", ["pending", "confirmed"])
