    ...
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>{event ? DAY_TYPE_LABELS[event.dayType] : ""}</DialogDescription>
        </DialogHeader>
        {event && <EventDetail event={event} isLoggedIn={isLoggedIn} />}
      </DialogContent>
    </Dialog>
  );
}
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <Badge className={cn(STATUS_STYLES[event.status])}>{event.status}</Badge>
        <Badge
          style={{
      ...
      <div className="space-y-2 text-sm">
        ...
        <div>
          <span className="font-medium">Start:</span> {formatDateTime(event.start, event.allDay)}
        </div>
        {!event.allDay && (
          <div>
            <span className="font-medium">End:</span> {formatDateTime(event.end, false)}
          </div>
        )}
        ...
        {isLoggedIn ? (
          <>
            ...
            {event.expectedAttendance != null && (
              <div>
                <span className="font-medium">Expected Attendance:</span> {event.expectedAttendance}
              </div>
            )}
          </>
        ) : (
          <p className="italic text-muted-foreground">Log in to view booking details.</p>
        )}
      </div>
    </div>
  );
