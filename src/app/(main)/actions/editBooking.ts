...
export async function editMultiDayBooking(input: EditMultiDayBookingInput) {
  ...
  if (datesChanged) {
    ...
    const approvers = originalBooking.roomId ? await getApprovers(originalBooking.roomId) : [];
...
export async function editStandardBooking(input: EditStandardBookingInput) {
  ...
  if (datesChanged) {
    ...
    if (conflicts.length > 0) {
      return {
        error: "The selected time slot conflicts with an existing booking.",
        success: false,
      };
    }
